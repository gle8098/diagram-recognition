...
class Recognizer:
    ...
    def find_edges(self, v_lines, h_lines):
        MIN_DIFF = 5

        up_line = h_lines[0]
        up_edge = np.sum(up_line[1] - v_lines[:, 1] > MIN_DIFF) < (v_lines.shape[0] / 1.5)

        down_line = h_lines[-1]
        down_edge = np.sum(v_lines[:, 3] - down_line[1] > MIN_DIFF) < (v_lines.shape[0] / 1.5)

        left_line = v_lines[0]
        left_edge = np.sum(left_line[0] - h_lines[:, 0] > MIN_DIFF) < (h_lines.shape[0] / 1.5)

        right_line = v_lines[-1]
        right_edge = np.sum(h_lines[:, 2] - right_line[0] > MIN_DIFF) < (h_lines.shape[0] / 1.5)
